...
<template>
    ...
    <div class="relative sm:flex sm:justify-center sm:items-center min-h-screen bg-dots-darker bg-center bg-gray-100 dark:bg-dots-lighter dark:bg-gray-900 selection:bg-red-500 selection:text-white">
        <div v-if="canLogin" class="sm:fixed sm:top-0 sm:left-0 p-6 text-left">
            <template v-if="!($page.props.auth.user)">
                <Link :href="route('admin-login')" class="font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">Administrator</Link>
            </template>
        </div>
        ...
        <div class="max-w-7xl mx-auto p-6 lg:p-8">
            <img src="/img/voucher-logo-white.png" class="w-64 h-64" />

            <h2 class="mt-6 text-center text-xl font-semibold text-gray-900 dark:text-white">
                VOUCHER SYSTEM WEB APP
            </h2>
        </div>
    </div>
</template>
